...
const Menu = ({click,state}:{click:(state:boolean) => any,state:boolean}) => {
  const showNav = {
    show : {
      top:0,
      transition:{
  ...
  return (
    <motion.div className="fixed left-0 -top-[100%] bg-softblack h-[100svh] w-screen text-softwhite z-10 flex justify-between" variants={showNav} animate="show" exit="exit">
      ...
      <section className="w-[50%] p-32 relative">
        ...
        <motion.ul className="flex flex-col gap-4 mt-12" variants={staggerList} initial="hidden" animate="show" exit="exit">
          {
            navLinks.map((link,i) => (
              <motion.li key={`i_${i}`} className="overflow-hidden hover:underline underline-offset-2 max-w-max">
